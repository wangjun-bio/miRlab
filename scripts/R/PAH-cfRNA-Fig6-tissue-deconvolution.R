...
setwd("./data")
DATA_DIR <- "."
RESULT_DIR <- file.path("..", "result")
dir.create(RESULT_DIR, recursive = TRUE, showWarnings = FALSE)

# Load bulk and single-cell inputs
bk.dat=read.csv(file.path(DATA_DIR, "counts_no_combat_all_Deconvolution.csv"), header = T)
tissue.labels<- readLines(file.path(DATA_DIR, "tissue_label.txt"))
tissue.celltype.labels<- readLines(file.path(DATA_DIR, "tissue_celltype_label.txt"))
sc.dat=read_feather(file.path(DATA_DIR, "filtered_sc_counts_Deconvolution.feather"))
...
bk.dat=bk.dat[,colnames(bk.dat) %in% common_genes]
sc.dat=sc.dat[,colnames(sc.dat) %in% common_genes]
...
# QC: reference tissue correlation
plot.cor.phi (input=sc.dat,
              input.labels=tissue.labels,
              title="tissue correlation",
              pdf.prefix=file.path(RESULT_DIR, "tissue_no_combat"),
              cexRow=1.2, cexCol=1.2,
              margins=c(2,2))

# QC: detect outlier genes
sc.stat <- plot.scRNA.outlier(
  input=sc.dat,
  cell.type.labels=tissue.labels,
  species="hs",
  return.raw=TRUE,
  pdf.prefix=file.path(RESULT_DIR, "sc_data_tissue_outlier") )
saveRDS(sc.stat, file.path(RESULT_DIR, "tissue_sc_stat.rds"))

# QC: filter noisy genes
...
plot.bulk.vs.sc (sc.input = sc.dat.filtered,
                 bulk.input = bk.dat,
                 pdf.prefix=file.path(RESULT_DIR, "sc_bulk_Tissue_filtered_gene") )

# Select protein-coding genes
sc.dat.filtered.pc <-  select.gene.type (sc.dat.filtered,
                                         gene.type = "protein_coding")

# Select tissue marker genes
markers <- readLines(file.path(DATA_DIR, "tissue_marker_top100.txt"))
...
myPrism <- new.prism(
  ...
  input.type="count.matrix",
  cell.type.labels = tissue.labels,
  cell.state.labels = tissue.celltype.labels,
  key=NULL,
  outlier.cut=0.01,
  outlier.fraction=0.1,
)
saveRDS(myPrism, file.path(RESULT_DIR, "Tissue_marker_bayes_filtered_gene.rds"))

# Run BayesPrism deconvolution
bp.res <- run.prism(prism = myPrism, n.cores=100)
saveRDS(bp.res, file.path(RESULT_DIR, "result_bayes_Tissue_marker_100.rds"))


# Configure downstream analysis
config <- list(
  sampleid_file    = file.path(DATA_DIR, "WGCNA/after-combat/sampleid.rds"),
  sampleMEs_file   = file.path(DATA_DIR, "WGCNA/no_combat/sample_MEs_DEG.csv"),
  bayes_res_file   = file.path(DATA_DIR, "Tissue/result_bayes_Tissue_no_combat.rds"),

  tissue_color     = c(ggsci::pal_igv("default")(51), ggsci::pal_uchicago("default")(3)),
...
# Fig6-SF3-1: stacked sample-level tissue fractions
...
write.csv(df_long,file.path(RESULT_DIR, "Table_S8.csv"),row.names = T)

dir.create(config$output_dirs$barplot, recursive=TRUE, showWarnings=FALSE)
ggsave(
  filename = file.path(config$output_dirs$barplot, "Fig6-SF3-1.pdf"),
  plot     = p, width=12, height=8, units="in"
)
ggsave(
  filename = file.path(config$output_dirs$barplot, "Fig6-SF3-1.tif"),
...
# Fig6B-1: group-level mean tissue fractions
...
dir.create(config$output_dirs$barplot, recursive = TRUE, showWarnings = FALSE)
ggsave(
  filename = file.path(config$output_dirs$barplot, "Fig6B-1.pdf"),
  plot     = p_group, width = 14, height = 6, units = "in"
)
ggsave(
  filename = file.path(config$output_dirs$barplot, "Fig6B-1.tif"),
  plot     = p_group, width = 14, height = 6, units = "in",
  device   = "tiff", dpi = 300, compression = "lzw"
)

# Fig6-SF3-2: tissue-fraction boxplot by group
new_order <- df_long %>%
  group_by(tissue) %>%
...
ggsave(
  filename = file.path(config$output_dirs$boxplot, "Fig6-SF3-2.pdf"),
  plot     = p, width = 19, height = 8, units = "in"
)
ggsave(
  filename = file.path(config$output_dirs$boxplot, "Fig6-SF3-2.tif"),
  plot     = p, width = 19, height = 8, units = "in",
  device   = "tiff", dpi = 300
)

# Fold-change analysis against NOR
nor_means <- df_long %>%
  filter(group == "NOR") %>%
  group_by(tissue) %>%
...
# Fig6B-2: grouped key-tissue boxplots
df_other <- df_long %>%
  mutate(
    tissue = if_else(
      tissue %in% c("Heart", "Blood", "Lung", "Vasculature", "Skin"),
      tissue,
      "Other_Tissue"
    )
  )
ordered_levels <- c("Heart", "Blood", "Vasculature", "Skin", "Lung", "Other_Tissue")
...
dir.create(config$output_dirs$boxplot, recursive = TRUE, showWarnings = FALSE)
ggsave(
  filename = file.path(config$output_dirs$boxplot, "Fig6B-2.pdf"),
  plot     = p,
  width    = 12,
  height   = 3,
  units    = "in"
...
ggsave(
  filename = file.path(config$output_dirs$boxplot, "Fig6B-2.tif"),
  plot     = p,
  width    = 12,
  height   = 3,
  units    = "in",
...
group_levels <- c("NOR","IPAH","CHD","SLE")
# Fig6-SF5-1: tissue-fraction heatmap
...
pdf(file.path(config$output_dirs$heatmap, "Fig6-SF5-1.pdf"), width = 12, height = 8)
print(p_tissue)
dev.off()
tiff(file.path(config$output_dirs$heatmap, "Fig6-SF5-1.tif"), width = 12, height = 8, units = "in", res = 300)
print(p_tissue)
dev.off()
...
ggsave(file.path(config$output_dirs$pca,"Fig6C.pdf"),p_pca,width=8,height=8,units="in")
ggsave(file.path(config$output_dirs$pca,"Fig6C.tif"),p_pca,width=8,height=8,units="in",device="tiff",dpi=300)

sampleMEs <- read.csv(config$sampleMEs_file,header=TRUE,row.names=1)
common_samples <- intersect(rownames(sampleMEs),rownames(theta_tissue))
theta_subset <- theta_tissue[rownames(theta_tissue)%in%common_samples,] %>% as.data.frame()

# Fig6D: module-tissue correlation bubble plot
...
ggsave(file.path(config$output_dirs$bubble,"Fig6D.pdf"),p,width=10,height=12,units="in")
ggsave(file.path(config$output_dirs$bubble,"Fig6D.tif"),p,width=10,height=12,units="in",device="tiff",dpi=300)

# Fig6-SF6: key module-tissue correlation scatter plots
sampleME_key=sampleMEs[,colnames(sampleMEs) %in% c("MEmidnightblue","MEpurple","MEgreenyellow")]
theta_sub_key=theta_subset[,colnames(theta_subset) %in% c("Heart","Blood","Skin")]
...
ggsave(
  filename = file.path(out_dir, "Fig6-SF6.pdf"),
  plot     = p_all,
  width    = 10, height = 10, units = "in"
)
ggsave(
  filename = file.path(out_dir, "Fig6-SF6.tif"),
  plot     = p_all,
  width    = 10, height = 10, units = "in",
  device   = "tiff", dpi = 300
)

# Export single marginal-density scatter plots
