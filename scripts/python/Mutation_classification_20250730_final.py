# =============================================================================
# Binary Classification: PAH vs NOR using Transcript-Derived Genetic Variants
# =============================================================================
# This script:
#   1. Reads mutation presence matrix (RDS format) from ANNOVAR output
#   2. Reads sample IDs for PAH and NOR groups
...
# =============================================================================
DATA_DIR = "data"
OUTPUT_RESULTS_DIR = "results/ML/mutation"
OUTPUT_FIGURES_DIR = "figures/ML/mutation"

# Input files
SAMPLE_ID_PAH = os.path.join(DATA_DIR, "PAH_not4pnk_ID.txt")
SAMPLE_ID_NOR = os.path.join(DATA_DIR, "NOR_not4pnk_ID.txt")
MUTATION_MATRIX_RDS = os.path.join(DATA_DIR, "ALL_DP0_GT11_presence.rds")
...
# =============================================================================
# Main Analysis Function (Mutation-specific)
# =============================================================================
def analyze_mutation(X_train, X_test, y_train, y_test, stability_threshold=0.75,
                     n_bootstraps=100, return_metrics=False, return_features=False, iteration=None):
    """
    Feature selection (LASSO stability) and model training for mutation data.
    """
    train_auc_dict = {}
    train_acc_dict = {}
    lasso_all_coefs = []
...
def run_repeated_classification(data, r=30):
    """
    Run classification r times with stratified train-test splits on mutation data.
    """
    ...
    for i in range(r):
        ...
        acc_dict, train_auc_dict, train_acc_dict, selected, risk_df, coef_df = analyze_mutation(
...
# Read mutation presence matrix (RDS)
result = pyreadr.read_r(MUTATION_MATRIX_RDS)
df = result[None]

# Keep only variant identifier and sample columns (starting with 'GDM')
selected_columns = df.columns[(df.columns == 'variant') | df.columns.str.startswith('GDM')]
df_filtered = df[selected_columns]
df_filtered = df_filtered.set_index("variant")

# Transpose to have samples as rows, variants as columns
df_filtered = df_filtered.T

# Create metadata (sample group)
df_meta = pd.DataFrame(index=df_filtered.index)
...
data = pd.merge(df_filtered, df_meta, left_index=True, right_index=True, how='inner')
